...
2. You can identify the target interfaces by looking at the output of `wsr l` (list) and noting the interface names in the last column. Set the environment variables `INTERNET_IF_NAME` and `LAN_IF_NAME` accordingly. For example: `setx INTERNET_IF_NAME "Intel(R) Wi-Fi 6 AX201 160MHz"`. Alternatively, modify lines 5 and 7 of the `wsr.cmd` file.
3. Run `wsr e` (enable) ...
